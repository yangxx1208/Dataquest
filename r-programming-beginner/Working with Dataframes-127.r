...
delay <- subtract(dateReported,dateSighted)

print(length(dateReported))
...
filter <- positiveDelays
positiveDates <- dates[filter,]
...
delay <- subtract(cleanDates$dateReported, cleanDates$dateSighted)