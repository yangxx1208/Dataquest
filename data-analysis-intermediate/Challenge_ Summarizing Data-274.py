...
rg_cat_counts = dict()
def total_cate(data):
    major_categorys = data["Major_category"].unique()
    total_count = {}
    for category in major_categorys:
        row_category = data[data["Major_category"] == category]
        total_count[category] = row_category["Total"].sum()
    return total_count

aa_cat_counts = total_cate(all_ages)
rg_cat_counts = total_cate(recent_grads)
