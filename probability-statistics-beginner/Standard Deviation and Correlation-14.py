...
differences_two = [i-values_median for i in values]
median_difference_sum = sum(differences_two)
...
for i in nba_stats["pts"]:
    ...
    variance_difference = difference **2
    variance += variance_difference
...
def stand_dev(data):
    ...
    variance = 0
    for i in data:
        difference = i - data_mean
        variance_difference = difference ** 2
        variance += variance_difference
    deviation = (variance/len(data)) ** (1/2)
    return deviation

mp_dev = stand_dev(nba_stats["mp"])

ast_dev = stand_dev(nba_stats["ast"])
...
point_10_dis = point_10 - mean
point_10_std = point_10_dis/std_dev

point_100_dis = point_100 - mean
point_100_std = point_100_dis/std_dev
...
data_norm = norm.pdf(data, 0 , 2)

plt.plot(data, data_norm)
plt.show()
...
aa = cov(nba_stats["fta"], nba_stats["blk"])